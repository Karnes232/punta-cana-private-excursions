...
export function OperatorInquiryEmail(data: InquiryEmailData) {
  return (
    <Html>
      <Head />
      <Preview>
        [VIP] New private-experience inquiry — {data.excursionTitle} — {data.name}
      </Preview>
      <Body style={main}>
        <Container style={container}>
          <Section style={card}>
            <Text style={eyebrow}>VIP · Private Site</Text>
            <Heading style={h1}>New private-experience inquiry</Heading>
            <Text style={subtle}>
              {data.name} requested information about &ldquo;
              {data.excursionTitle}&rdquo;
